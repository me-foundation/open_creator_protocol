...
pub struct CloseCtx<'info> {
    #[account(
        constraint = mint_manager.mint == mint.key() @ ErrorCode::InvalidMint,
    )]
    ...
    #[account(mut, constraint =
        ...
        && holder_token_account.amount == 1 @ ErrorCode::InvalidHolderTokenAccount)]
...
pub fn handler(ctx: Context<CloseCtx>) -> Result<()> {
    if ctx.accounts.holder_token_account.is_frozen() {
